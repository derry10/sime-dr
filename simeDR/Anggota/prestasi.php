...
<script>
  // Fungsi untuk menampilkan detail prestasi
  $('#ubahDataPopup').on('show.bs.modal', function (event) {
    ...
    var modal = $(this)
    modal.find('.modal-body #detailNama').text(nama)
    modal.find('.modal-body #detailJenisKelamin').text(jenisKelamin)
    modal.find('.modal-body #detailJurusan').text(jurusan)
    modal.find('.modal-body #detailEkskul').text(ekskul)

    $.ajax({
      url: 'get_prestasi.php',
      type: 'post',
      data: {anggota_id: anggotaId},
      success: function(response) {
        $('#prestasiTableBody').html(response);
      }
    });
  });

  // Fungsi untuk menampilkan form tambah prestasi
  $('#tambahkanPrestasiModal').on('show.bs.modal', function (event) {
    var button = $(event.relatedTarget)
    var anggotaId = button.data('anggota-id')

    var modal = $(this)
    modal.find('.modal-body #tambahAnggotaId').val(anggotaId)
  });

  // Fungsi untuk menampilkan form edit prestasi
  $('#editPrestasiModal').on('show.bs.modal', function (event) {
    var button = $(event.relatedTarget)
    var prestasiId = button.data('prestasi-id')
    ...
    var modal = $(this)
    modal.find('.modal-body #editPrestasiId').val(prestasiId)
    modal.find('.modal-body #editKegiatan').val(kegiatan)
    modal.find('.modal-body #editTanggal').val(tanggal)
    modal.find('.modal-body #editTingkat').val(tingkat)
